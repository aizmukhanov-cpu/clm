...
function DealCard({
  ...
  function handleMove() {
    if (!next) return;
    startTransition(async () => { await moveDealStage(deal.id, next, team); });
  }

  function handleClose(outcome: "WON" | "LOST") {
    startTransition(async () => {
      const result = await closeDeal(deal.id, outcome, outcome === "LOST" ? lostReason || null : null, team);
      if (outcome === "WON" && result?.clientId) {
        setWonClientId(result.clientId);
      }
    });
  }
  ...
  return (
    <div className="bg-white rounded-lg border border-gray-100 shadow-sm p-3 text-xs space-y-2">
      ...
      <div className="flex items-center gap-3 text-gray-400">
        ...
        {deal.expectedClose && (
          <span>до {format(new Date(deal.expectedClose), "dd.MM")}</span>
        )}
...
function NewDealForm({
  ...
  const action = createDeal.bind(null, team);
    ...
    <div className="fixed inset-0 bg-black/40 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-xl shadow-xl w-full max-w-md p-6">
        <div className="flex items-center justify-between mb-4">
          <h3 className="font-semibold text-gray-900">Новая сделка</h3>
          <button onClick={onClose} className="text-gray-400 hover:text-gray-600 text-xl leading-none">×</button>
        </div>

        <form action={formAction} className="space-y-3 text-sm">
          <div>
            <label className="block text-xs text-gray-500 mb-1">Название лида / компания</label>
            <input
              name="leadName"
          ...
          return (
            <div key={stage} className="flex flex-col gap-3">
              ...
              <div className="space-y-2.5 min-h-[120px]">
                {colDeals.map((deal) => (
                  <DealCard key={deal.id} deal={deal} team={team} />
                ))}
                {colDeals.length === 0 && (
